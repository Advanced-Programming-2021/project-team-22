...
public class MainMenuView extends Application {
    ...
    private void handleCreateCardButton() {
        createCardButton.setOnMouseClicked(mouseEvent -> {
            try {
                if (MainMenuController.getLoggedInPlayer().isPlaySFX()) Utils.playButtonClickSFX();

                CreateCardMenuController.setLoggedInPlayer(MainMenuController.getLoggedInPlayer());
                new CreateCardMenuView().start(Utils.getStage());
            } catch (Exception exception) {
                exception.printStackTrace();
            }
        });
